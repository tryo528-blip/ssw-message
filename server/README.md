...
> ### ⚠ 이 코드는 CONTRACT **v2** 구현입니다
> 규격은 [v3](../docs/CONTRACT.md) 로 올라갔고, 코드는 아직 따라가지 않았습니다.
> 동작에는 문제가 없지만(검사 9/9 통과) 아래가 미반영입니다.
>
> | v3 항목 | 현재 코드 |
> |---|---|
> | `submission_id` / `record_id` 분리 | `id` 하나로 겸용 |
> | `content_digest` | 없음 |
> | 상태 조회 API (`/api/status`) | 없음 |
> | 오류 코드 4종 (`INVALID_MEDIA` 등) | 없음 |
> | 바이너리 전송 | base64 JSON |
> | 검증/보정 2단계 분리 | 단일 단계 |
>
> 동기화는 P2 과제입니다. 그 전까지 이 프로토타입은 **암호화·전송·복호화 경로를
> 증명하는 용도**로만 쓰세요.
