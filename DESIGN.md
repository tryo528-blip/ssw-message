...
|---|---|
| 작성 | 2026-08-17 |
| 개정 | v0.2 — 개인정보 요건 반영, 전송 경로 전면 교체 |
| 전송 경로 | **확정: 사무실 직접 수신 + E2E 암호화** (사무실 네트워크 조건 충족 확인) |
| 관련 문서 | [docs/TRANSPORT.md](docs/TRANSPORT.md) · [docs/CONTRACT.md](docs/CONTRACT.md) · [server/README.md](server/README.md) |
...
사무실 PC에 별도 설치할 런타임은 Node와 Caddy뿐입니다.
...
|---|---|---|
| P0-a | 설계·연동 규격 | ✅ 완료 |
| P0-b | 수신 서버 + 복호화 + 검사 9종 | ✅ 완료 (전부 통과) |
| **P0-c** | **카메라 갤러리 미저장 실기 검증** | 🔴 **진행 중 — 최대 리스크** |
| P1 | 사무실 설치 (DDNS·포트포워딩·Caddy·자동시작) | 대기 |
| P2 | PWA 본체 (메모 → 카메라 → 암호화 → 전송) | 대기 |
| P3 | 오프라인 대기열, 전송 로그, QR 등록, 설정 | 대기 |
| P4 | ERP 수집기 연결 + 보존정책 + 운영 매뉴얼 | 대기 |

**P0-c가 프로젝트 성패입니다.** 아이폰·갤럭시 실기에서 촬영 후 갤러리를 직접 확인해
"안 남는다"가 확인돼야 P2로 갑니다. 여기서 실패하면 설계 전제가 깨집니다.
...
2. ERP 수집기 연결 방식 — .NET / Python / ERP 내장 스케줄러
3. 사진 보존기간 — 사용처(현장 증빙 / 서류 / 신분 관련)에 따라 결정
