...
> P0-c 갤럭시(삼성인터넷) 실기 통과. 아이폰·홈화면 추가는 잔여.
...
`-webkit-touch-callout:none`, `pointer-events:none`, 전송 후 즉시 참조 해제.
`<img src=blob>` 과 다운로드 링크는 쓰지 않습니다 — iOS 길게 누르기의 저장 대상이 됩니다.

iOS 26 / Safari 26 문헌 (실기 전 적용, 상세는 HANDOFF §3):
홈화면 추가는 기본이 웹앱. UA OS 버전은 `18_6`으로 고정되어 파싱 금지.
getUserMedia는 동시에 1개, 백그라운드에서 트랙을 끊고, 해시 라우팅으로 권한을 다시 묻지 말 것.
캡처는 canvas만 (Safari는 `ImageCapture.takePhoto` 없음).

**한계 — 스크린샷은 막을 수 없습니다.** 웹이든 네이티브든 iOS에서는 불가능합니다.
운영 규칙으로 관리해야 합니다. 갤러리 미저장(C1)은 아이폰 실기가 오기 전에는 닫지 않습니다.
...
| 사진 로컬 잔존 | 판단 2 (파일시스템 미경유) | 갤럭시 삼성인터넷 통과 · 아이폰 대기 |
...
└─ app/                       # PWA (Vite + React). envelope.mjs 를 그대로 import
```
...
| **P0-c** | **카메라 갤러리 미저장 실기 검증** | ✅ 갤럭시 통과 · 아이폰 잔여 |
| P2 | 서버 v3 동기화 — 식별자 분리, 오류 코드, 상태 조회, 바이너리 전송 | ✅ **완료 (34/34)** |
| P1 | 미니PC 구매 + 설치 (Ubuntu·LUKS·DDNS·Caddy·systemd) | 대기 |
| P3 | PWA 본체 (메모 → 카메라 → 암호화 → 전송 → 확인 → 삭제) | ✅ 본체 있음 · QR은 P5 |
| P4 | 문서 보정 파이프라인 (Python + OpenCV) | ✅ scan.py · 확신 없으면 skip |
| P5 | 대기함 UI, QR 등록, 설정 | ✅ JSON+QR 스캔 · 대기함. 홈화면 설치는 잔여 |
| P6 | ERP 수집기 연결 + 운영 도구(로그·디스크·자동삭제) | 운영 도구 ✅ · ERP 대기 |

**P0-c 갤럭시(2026-08-17):** 삼성인터넷 30 · 방식 A 갤러리 없음 · EXIF 없음 · 암호화 12ms.
아이폰 Safari와 홈화면 추가(standalone)는 아직이다. 갤럭시에서 전제가 깨지지는 않았다.

P1을 먼저 하면 아이폰 검증을 실환경 HTTPS에서 할 수 있습니다
